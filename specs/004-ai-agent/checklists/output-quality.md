...
- [ ] CHK002 - `sources` フィールドに含めるべき URL の品質基準（公式ドキュメントの
  定義、ブログ記事は可か不可か、バージョン付き URL が必要か）は定義されているか
  [Completeness] [Spec §FR-006, §SC-003]
- [ ] CHK003 - 各プロバイダーにつき最低 1 つの公式 URL（SC-003）とあるが、
...
- [ ] CHK005 - 「JSON Schema に準拠した result.json」の準拠レベルは明確か。
...
- [ ] CHK009 - SC-003「各プロバイダーの公式 URL が最低 1 つ」と FR-006 の
...
- [ ] CHK010 - 比較対象プロバイダーが 2 社の場合と 3 社以上の場合で、
...
- [ ] CHK012 - 比較項目の値が「該当なし」「情報なし」の場合の表現方法
